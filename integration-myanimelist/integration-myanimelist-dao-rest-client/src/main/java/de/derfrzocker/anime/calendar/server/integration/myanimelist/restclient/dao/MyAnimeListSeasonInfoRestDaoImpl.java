...
@ApplicationScoped
public class MyAnimeListSeasonInfoRestDaoImpl implements MyAnimeListSeasonInfoDao {

    private static final Logger LOG = Logger.getLogger(MyAnimeListSeasonInfoRestDaoImpl.class);
    ...
    private static final String UNEXPECTED = "Could not get MyAnimeList season info for year '%s' and season '%s' for, because of unexpected status '%s'.";
    ...
    private RestResponse<AnimeListResponseTDO> getResponse(Year year, Season season) {
        return this.restClient.getSeasonAnime(year.getValue(),
